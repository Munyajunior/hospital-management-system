...
async def predict_admissions(db: Session = Depends(get_db)):
...
async def get_no_show_rate(db: Session = Depends(get_db)):
...
async def detect_anomalies_in_vitals(db: Session = Depends(get_db)):
...
async def detect_anomalies_in_lab_tests(db: Session = Depends(get_db)):
    ...
    anomalies = await ai.detect_anomalies_in_vitals()  # Reuse the same method for simplicity
    return {"anomalies": anomalies.tolist()}

@router.get("/detect-anomalies/radiology-scans", response_model=Dict[str, List[int]])
async def detect_anomalies_in_radiology_scans(db: Session = Depends(get_db)):
