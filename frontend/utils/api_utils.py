...
def post_data(self, api_url, data, auth_token=None):
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            QMessageBox.critical(self, "Error", f"Failed to post data to {api_url}")
            return False
    except Exception as e:
        QMessageBox.critical(self, "Error", f"An error occurred: {e}")
        return False
...
def update_data(self, api_url, data, auth_token=None):
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            QMessageBox.critical(self, "Error", f"Failed to update data to {api_url}")
            return False
    except Exception as e:
        QMessageBox.critical(self, "Error", f"An error occurred: {e}")
        return False
...
def delete_data(self, api_url, auth_token=None, params=None):
    ...
    try:
        ...
        if response.status_code == 204:  
            ...
            QMessageBox.critical(self, "Error", f"Failed to delete data. Error {response.status_code}")
        return False
    except Exception as e:
        QMessageBox.critical(self, "Error", f"An error occurred: {e}")
        return False
